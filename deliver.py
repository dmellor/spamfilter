...
class Deliver(ConfigMixin):
    ...
    def process(self):
        ...
        session = createSession(self.getConfigItem('database', 'dburi'))
        try:
            query = session.query(SpamRecipient).filter_by(
            ...
            spam = session.query(Spam).get(spam_recipient.spam_id)
            ...
            session.delete(spam_recipient)
            query = session.query(SpamRecipient)
