...
# sending the responses from the server back to the client. The proxy should be
# set to run via the Postfix spawn service, which means that all communication
# with the SMTP client will be performed via stdin and stdout. The proxy
# examines the contents of the DATA command sent by the SMTP client, and either
# passes the contents on to the SMTP server or terminates the network
# connection to the SMTP server. In this latter case, it still reports to the
# SMTP client that the mail has been accepted - this behaviour is required in
# order to implement an after-queue content filter for Postfix. A subclass of
# this class would override the checkMessage method to qurantine the message if
# it is identified as spam or if it contains a virus.

package AfterQueueProxy;
